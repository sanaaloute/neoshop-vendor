...
export function OrderDetailDrawer({
  ...
  const loadTracking = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        setTrackingError(httpErrorMessageForUser(e, t("couldNotLoadOrder")));
      } finally {
  ...
  }, [open, orderId, refreshOrderFromApi, loadTracking]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <Tabs defaultValue="overview" className="flex flex-1 flex-col">
          ...
          <TabsContent value="overview" className="flex flex-1 flex-col gap-4 px-4 py-4">
            <section>
                ...
                {order.lines.map((l) => (
                  ...
                  >
                    ...
                    <span className="shrink-0 text-right tabular-nums">
                      ×{l.qty}
                      <br />
                      {formatCurrency(toNumber(l.unitPrice) * l.qty)}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <TabsContent value="tracking" className="flex flex-1 flex-col gap-4 px-4 py-4">
            <section>
              <h3 className="text-muted-foreground mb-2 flex items-center gap-1.5 text-xs font-semibold tracking-wide uppercase">
                <Truck className="size-3.5" aria-hidden />
                {t("trackingTab")}
              ...
              {trackingLoading ? (
                <p className="text-muted-foreground text-sm">{t("loadingOrder")}</p>
              ) : trackingError ? (
                ...
                <div className="space-y-3">
                  <Card className="border-border/70 bg-muted/15 p-3">
                    <dl className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <dd className="font-medium">{trackingData.carrier || "—"}</dd>
                      </div>
                      <div>
                        <dt className="text-muted-foreground text-xs">{t("tracking")}</dt>
                        <dd className="font-mono font-medium">{trackingData.trackingNumber || "—"}</dd>
                      </div>
                    </dl>
                  </Card>
                  {trackingData.events.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-xs">{t("noTrackingData")}</p>
                  )}
                </div>
              ) : (
                <p className="text-muted-foreground text-xs">{t("noTrackingData")}</p>
