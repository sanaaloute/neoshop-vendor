...
**Frontend flow:**
1. Show a "Resend email" button with a cooldown timer (e.g., 60 seconds)
2. Call `POST /auth/resend-verification`
3. Show a success toast: "If an account exists, a verification email has been sent"
...
### 1.6 Password Recovery (Email Only)
...
### 1.7 Change Email (Authenticated)
...
**Frontend flow:**
1. Collect `newEmail` and current `password` from the user
2. Call `POST /auth/change-email`
3. Show success: "A confirmation email has been sent to your new address"
4. Supabase sends a confirmation email to the **new** address
5. User clicks the confirmation link in the new email
6. The user must **log in again** after confirming — the new email becomes active immediately in Supabase, and the backend syncs it on the next login or authenticated request

**Security note:** Current password is required to prevent session hijackers from changing the email.
...
- `emailVerifiedAt` is `null` until the user confirms their email via Supabase
- It syncs automatically on login and on every authenticated request
- Use this field to show/hide "verify your email" banners or gate certain features

---

## Part 4 — Session Management

### 4.1 Register / Refresh a Server Session
...
### 4.2 Refresh an Expired Access Token
...
| `POST /auth/resend-verification` | 3 | 60 seconds |
| `POST /auth/change-email` | 3 | 60 seconds |
...
| Email already in use (change email) | 422 | `Email address is already in use or invalid` |
| Invalid or expired session (change email) | 401 | `Invalid or expired session` |
...
| User Type | Registration | Login | Password Reset | Email Change |
|-----------|-------------|-------|----------------|--------------|
| **Customer / Vendor** | Email (with verification) OR Phone OTP | Email + password OR Phone OTP | Email link via `/auth/forgot-password` | `POST /auth/change-email` |
| **Admin** | One-time bootstrap (`/setup/bootstrap-admin`) only | Email + password | Email link via `/auth/forgot-password` | `POST /auth/change-email` |
...
- [ ] Resend verification: implement cooldown timer (60s) on "Resend email" button
...
- [ ] Change email: require current password, show "check your new email" confirmation
- [ ] Read `emailVerifiedAt` from `GET /auth/me` to show/hide verification banners
